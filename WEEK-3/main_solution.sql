...
-- I have changed some column names because names like ORDER and DATE are special SQL keywords. Keeping them as column names can cause errors, so I renamed them to make them easier to use and understand..ALTER TABLE superstore1.superstore_raw
ALTER TABLE superstore1.superstore_raw
...
CREATE TABLE superstore1.customers (
...
INSERT INTO superstore1.customers (customer_id,
				customer_name,
                segment)
SELECT DISTINCT
    customer_id,
    customer_name,
    segment
FROM superstore1.superstore_raw;

-- Creaded the product table 
CREATE TABLE superstore1.products (
...
INSERT INTO superstore1.products (
    product_id,
    product_name,
    category,
    sub_category
...
-- Created the Orders table with foreign key constraints to establish relationships between the tables and maintain referential integrity 
CREATE TABLE superstore1.orders (
...
INSERT INTO superstore1.orders (order_id, row_id, order_date, ship_date, ship_mode, customer_id, product_id, sales, quantity, discount, profit)
SELECT DISTINCT
    order_id,
    row_id,
    order_date,
...
-- First I found the average sales from the order table. Then, I used it as a subquery to find all the required records.

SELECT * FROM superstore1.orders
where sales > (SELECT AVG(sales) FROM superstore1.orders);
...
SELECT
    ...
    product_id,
    sales
FROM superstore1.orders o
WHERE sales = (
    SELECT MAX(sales)
    FROM superstore1.orders p
...
-- ROW_NUMBER() then gives a sequence number to each order of that customer based on order_date.
SELECT customer_id,
    order_id,
    order_date,
    sales,
...
-- The query first calculates the total sales of each customer using SUM(sales) and GROUP BY customer_id.
-- Then, RANK() gives ranks based on total sales in descending order, and only the top 3 ranked customers are displayed.
...
JOIN superstore1.customers c 
ON o.customer_id = c.customer_id
WHERE o.sales > (SELECT AVG(sales) FROM superstore1.orders);
